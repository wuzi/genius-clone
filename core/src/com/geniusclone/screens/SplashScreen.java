...
public class SplashScreen implements Screen {
    ...
    public SplashScreen(Game g) {
        ...
        splashSound = Gdx.audio.newSound(Gdx.files.internal("sounds/splashsound.mp3"));
    }
    ...
    public void render(float delta) {
        ...
        if (TimeUtils.millis() > rendCount && opacity < 1.0f) {
            ...
            if (opacity > 1.0f)
                opacity = 1.0f;
        } else if (TimeUtils.millis() > (startTime + 5000)) {
    ...
    public void show() {
        splashSound.play();
        texture = new Texture(Gdx.files.internal("images/splash.jpg"));
